...
// Sets the child trees of a quadtree
func (qt *QuadTree) SetChildTrees() {
	if (qt.Boundary.HalfHeight >= TILE_SIZE) && (qt.Boundary.HalfWidth >= TILE_SIZE) {
		parentCenter := qt.Boundary.Center
		childHalfSide := max((qt.Boundary.HalfHeight / 2), (qt.Boundary.HalfWidth / 2))
		northWest := getChildNode(-1, -1, parentCenter, childHalfSide)
		northEast := getChildNode(1, -1, parentCenter, childHalfSide)
		southWest := getChildNode(-1, 1, parentCenter, childHalfSide)
		southEast := getChildNode(1, 1, parentCenter, childHalfSide)

		qt.NorthWest = &northWest
		qt.NorthEast = &northEast
		qt.SouthWest = &southWest
		qt.SouthEast = &southEast

		qt.NorthWest.SetChildTrees()
		qt.NorthEast.SetChildTrees()
		qt.SouthWest.SetChildTrees()
		qt.SouthEast.SetChildTrees() // recursive calls until node size reaches tile size
	}
}
...
func (qt *QuadTree) setWallTiles() {
	...
	for i := 0; i < MAP_TILE_HEIGHT; i++ {
		...
		for j := 0; j < MAP_TILE_WIDTH; j++ {
			...
			matchingQuadrant := qt.GetTileOfPoint(cellCenter)
			if lineCells[j] == "*" {
				matchingQuadrant.IsWall = true
				fmt.Print("██")
			} else {
				fmt.Print("  ")
			}
